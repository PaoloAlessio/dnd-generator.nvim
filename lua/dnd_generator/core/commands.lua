...
function M.sync_cache()
	...
	for _, endpoint in ipairs(valid_DNDFind) do
		client.get_data(endpoint, function(results)
			vim.schedule(function()
				if not results or #results == 0 then
					error_count = error_count + 1
          vim.notify("DND Generator: failed to sync " .. endpoint, vim.log.levels.WARN)
				else
					success_count = success_count + 1
				end

        if (success_count+error_count) == total then
          if error_count > 0 then
            vim.notify(
              "DND Generator: Sync terminated with errors (" .. success_count .. "/" .. total.." completed)",
            vim.log.levels.WARN)
          else
            vim.notify("DND Generator: Synchronization ended successfully", vim.log.levels.INFO)
          end
        end
			end)
		end)
		...
		elseif current_node.children ~= nil and current_node.children[args[i]] ~= nil then
			...
			elseif current_node.type == "file" then
				...
				if NPC_file_name[1] ~= nil then
					vim.notify(
						"ERROR: the file from where to select a name has been specified multiple times",
